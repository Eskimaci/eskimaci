...
def get_ndvi_for_year(year, target_month_start, target_month_end, config, geometry, size):
    ...
    try:
        ...
        print(f"DEBUG [{year}]: Tvar dát: {ndvi_array.shape}, Min: {np.nanmin(ndvi_array):.4f}, Max: {np.nanmax(ndvi_array):.4f}, Priemer: {np.nanmean(ndvi_array):.4f}")
        return ndvi_array
    except Exception as e:
...
def generate_trend_map(years_to_analyze, target_month_start, target_month_end):
    ...
    """
    print(f"--- Spúšťam dlhodobú analýzu NDVI trendu pre roky {years_to_analyze} a obdobie {target_month_start}-{target_month_end} ---")
    
    if not sh_config.sh_client_id:
        raise Exception("Chyba konfigurácie: Sentinel Hub Client ID nie je nastavené.")

    yearly_ndvi_data = [get_ndvi_for_year(year, target_month_start, target_month_end, sh_config, AOI_GEOMETRY, OUTPUT_SIZE) for year in years_to_analyze]
    
    # Odfiltrujeme roky, pre ktoré sa nepodarilo stiahnuť dáta
    valid_years_data = [(year, data) for year, data in zip(years_to_analyze, yearly_ndvi_data) if data is not None and not np.isnan(data).all()]
    ...
    print(f"DEBUG: Štatistiky mapy trendov - Min: {np.nanmin(trend_map):.4f}, Max: {np.nanmax(trend_map):.4f}, Priemer: {np.nanmean(trend_map):.4f}")
