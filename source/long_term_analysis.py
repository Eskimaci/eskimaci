...
def save_satellite_image(year, start_month, start_day, end_month, end_day, period_name, config, geometry, size,
                         park_name):
    ...
    try:
        ...
        # Uložíme obrázok
        rgb_image = data[0]
        safe_park_name = park_name.replace(' ', '_').replace('á', 'a').replace('ô', 'o').replace('ý', 'y').replace('ž',
                                                                                                                   'z')
